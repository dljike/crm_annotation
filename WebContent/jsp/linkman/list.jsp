...
<BODY>
	<FORM id="customerForm" name="customerForm" action="${pageContext.request.contextPath }/linkman/findLinkManList" method=post>		
									...
									<TD>
										<TABLE id=grid style="BORDER-TOP-WIDTH: 0px; FONT-WEIGHT: normal; BORDER-LEFT-WIDTH: 0px; BORDER-LEFT-COLOR: #cccccc; BORDER-BOTTOM-WIDTH: 0px; BORDER-BOTTOM-COLOR: #cccccc; WIDTH: 100%; BORDER-TOP-COLOR: #cccccc; FONT-STYLE: normal; BACKGROUND-COLOR: #cccccc; BORDER-RIGHT-WIDTH: 0px; TEXT-DECORATION: none; BORDER-RIGHT-COLOR: #cccccc" cellSpacing=1 cellPadding=2 rules=all border=0>
											<TBODY>
												...
												<c:forEach items="${pb.datas }" var="linkman">
												<TR
												...
												<%-- <TD>${linkman.customer.cust_name}</TD>--%> 
													<TD>
													<a href="${pageContext.request.contextPath }/linkMan/LinkManUpdate?lkm_id=${linkman.lkm_id}">修改</a>
													&nbsp;&nbsp;
													<a href="${pageContext.request.contextPath }/linkMan/LinkManDelete?lkm_id=${linkman.lkm_id}">删除</a>
													</TD>  
												</TR>
												</c:forEach>
											</TBODY>
									...
									<TD>
										<SPAN id=pagelink>
											<DIV style="LINE-HEIGHT: 20px; HEIGHT: 20px; TEXT-ALIGN: right">
												共[<B>${pb.total}</B>]条记录,[<B>${pb.totalPage}</B>]页
												,每页显示
												<select name="pageSize" onchange="chagePageSize()">
												<option value="3" <c:if test="${pb.pageSize==3 }">selected</c:if>>3</option>
												<option value="5" <c:if test="${pb.pageSize==5 }">selected</c:if>>5</option>
												</select>
												条
												[<A href="javascript:to_page(${pb.cuurentPage-1})">前一页</A>]
												<B>${pb.cuurentPage}</B>
												<c:if test="${pb.cuurentPage != pb.totalPage  }">
													[<A href="javascript:to_page(${pb.cuurentPage+1})">后一页</A>] 
												</c:if>
