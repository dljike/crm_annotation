...
<script type="text/javascript">
	$(function(){
		//所属行业 
		$.ajax({
			url:"${pageContext.request.contextPath }/customer/findCustomerList.action",
			type:"POST",
			dataType:"json",
			success:function(data){
				alert(data);
				for(var i = 0; i < data.length;i++){
					$("#cust_industry").append("<option value='"+data[i].cust_id+"'>"+data[i].cust_name+"</option>");
				}
			}
		});
...
<BODY>
	<FORM id=form1 name=form1 action="${pageContext.request.contextPath }/linkMan/save.action" method=post>
					...
					<TD vAlign=top width="100%" bgColor=#ffffff>
						...
						<TABLE cellSpacing=0 cellPadding=5  border=0>
							<tr>
								<td>所属客户：</td>
								<td >
									<select name="customer.cust_id" class=textbox id="cust_industry" style="WIDTH: 180px">
